...
const UploadPage = () => {
	...
	const { addUploadingVideo } = useVideoUploadContext();
	...
	useEffect(() => {
		...
		socket.on("uploadComplete", (data) => {
			console.log("progress done:", data);
	...
	const handleSubmit = async (e) => {
		...
		addUploadingVideo(tempId); // Notify VideosTab to show loader
		...
		try {
			...
			toast({ title: "Upload successful", description: `Your video "${response.data.data.title}" has been uploaded.` });

			updateUploadedVideo(tempId, response.data.data);
		} catch (error) {
			toast({ title: "Upload failed", description: error.response?.data?.error || "An error occurred.", variant: "destructive" });
