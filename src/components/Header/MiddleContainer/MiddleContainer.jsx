...
import { SHOW_SEARCH_BOX_BREAKPOINT, useIsMobileView, handleSearchFormSubmit as querySearchOnReload } from "../../../utils/utils";
...
const MiddleContainer = () => {
	...
	const showSearchBox = useMediaQuery(`(min-width: ${SHOW_SEARCH_BOX_BREAKPOINT}px)`);
	const [isSearchDrawerOpen, setIsSearchDrawerOpen] = useState(false);

	const currentLocation = useLocation();
	const isInSearchResultsPage = currentLocation.pathname === "/results";
	// retrieve the search term appears in URL
	const searchTermFromUrl = new URLSearchParams(currentLocation.search).get("search_query");
		...
		return (
			<StyledMiddleContainer>
				{isInSearchResultsPage ? (
					<MobileSearchTermContainer {...{ setIsSearchDrawerOpen, searchTermFromUrl }} />
				) : (
					<SearchButton {...{ setIsSearchDrawerOpen }} />
				)}
				<MobileViewSearchDrawer {...{ isSearchDrawerOpen, setIsSearchDrawerOpen }} />
			</StyledMiddleContainer>
		);
	} else {
		return (
